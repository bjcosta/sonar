...
class MergedPosition(object):
    def __init__(self, max_len): 
        ...
        self.water_depth = numpy.zeros(max_len)
    ...
    def MergePoint(self, 
        ...
        existing_water_depth = self.water_depth[index]
        existing_count = self.contributors[index]

        merged_contributors = existing_count + new_contributors
        merged_water_depth = float(((existing_water_depth * existing_count) + new_water_depth)) / merged_contributors
        
        self.water_depth[index] = merged_water_depth
    ...
    def ToXArray(self): 
        return xarray.Dataset({
            ...
            'water_depth': (['position'], self.water_depth[0:self.size]),
...
def MergePositionData(current_sonar_data, position_data):
    ...
    for index in range(0, len(current_sonar_data.position.values)):
        ...
        new_water_depth = current_sonar_data.water_depth.values[index]
        ...
        # Next see if it is the same as the last item in new_merged_data, as that will also be very common and fast to find
            ...
            if already_exists_in_position_data:
                ...
                old_merged_data.MergePoint(
                    ...
                    position_data.water_depth.values[position_data_index],
...
def MergeSonarLogByPosition(sonar_data, position_data):
    ...
    if 'water_depth' not in position_data.keys():
        ...
        ds = xarray.Dataset({
            ...
            'water_depth': (['position'],  numpy.array([], dtype=numpy.float32)),
    ...
    merged_postion_channel = xarray.Dataset({
            ...
            'water_depth': (['position'],  channel.water_depth),
            'contributors': (['position'],  numpy.ones(len(channel.water_depth))),
...
def GenerateSurfaceMeshFromPositionData(position_data, include_corners=False):
    if len(position_data) == 0 or len(position_data.longitude) == 0:
    ...
    water_depth_range = [position_data.water_depth.min().values.item(), position_data.water_depth.max().values.item()]

    offset = [longitude_range[0], latitude_range[0], 0]
    
    # Scale z axis to make it more easily visible in the mesh for now
    # We will choose scaling only INCREASE to make it at least 1/5 the x/y dists
    min_xy_dist = min(longitude_range[1]-longitude_range[0], latitude_range[1]-latitude_range[0])
    min_z_dist = min_xy_dist / 5
    z_dist = water_depth_range[1] - water_depth_range[0]
    z_scale = 1
    ...
    tmp_z = numpy.zeros(len(position_data.water_depth) + extra_points)
    ...
    for index in range(0, len(position_data.longitude)):
        ...
        tmp_z[index] = -1 * (position_data.water_depth.values[index] - offset[2]) * scale[2]
    ...
    surface_mesh.water_depth_range = water_depth_range
...
def GenerateDepthMapImageFromPositionData(position_data):
    if len(position_data) == 0 or len(position_data.longitude) == 0:
        logger.warning('No data to generate map from')
        return None
    
    longitude_range = [position_data.longitude.min().values.item(), position_data.longitude.max().values.item()]
    latitude_range = [position_data.latitude.min().values.item(), position_data.latitude.max().values.item()]
    data = (position_data.longitude.values, position_data.latitude.values, position_data.water_depth.values)
    #import code
    #code.interact(local=dict(globals(), **locals()))
    ...
    points = holoviews.Points(data, vdims='water_depth', kdims=[
        holoviews.Dimension('easting'), 
        holoviews.Dimension('northing')]).opts(color='water_depth', cmap='viridis').opts(**dimensions)
...
def SaveMergedData(merge_data_file_name, merged_data):
    dir = os.path.dirname(merge_data_file_name)
