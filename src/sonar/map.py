...
def AdjustToIncludeNearestLandmark(lon_min, lon_max, lat_min, lat_max):
    ...
    middle_lon_degrees, middle_lat_degrees = sonar.lowrance_log_parser.meters_to_lnglat(middle_lon, middle_lat)
    ...
    # @todo Add the nearest landmark to the cache meta-data and use it if already exists
    ...
    while node is None and radius <= max_radius:
        ...
        try: result = api.query(query)
            ...
            logger.warning('Failed to query nearby suburb: %s', e)
            break
        ...
        for i in range(0, len(result.nodes)):
            ...
            n = result.nodes[i]
            n_lon, n_lat = sonar.lowrance_log_parser.lnglat_to_meters(float(n.lon), float(n.lat))
        ...
        if min_dist_index is not None:
            ...
            node_lon, node_lat = sonar.lowrance_log_parser.lnglat_to_meters(float(node.lon), float(node.lat))
